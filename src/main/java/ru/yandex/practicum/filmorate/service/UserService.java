...
public class UserService {
    ...
    public Collection<User> getAllFriends(long userId) {
        ...
        return user.getFriends().stream().map(this::findByIdUser).collect(Collectors.toList());
    }

    public Collection<User> getCommonFriends(long userId, long otherId) {
        User user = findByIdUser(userId);
        User other = findByIdUser(otherId);
        return user.getFriends().stream().filter(other.getFriends()::contains).map(this::findByIdUser).collect(Collectors.toList());
    }
